                                   ...
                                   DrawMode_t mode)
{
    ...
    // LOCAL PATCH (foolscap): two fixes vs upstream.
    // (1) honor `mode`: a set bit in lut_1bpp is the source-driver code "01"
    //     (darken, matching the 4bpp conversion_lut where BLACK_ON_WHITE=0x55);
    //     the lighten code "10" is that shifted left one bit, so WHITE_ON_BLACK
    //     needs no extra table. Upstream ignored `mode` and only darkened.
    // (2) byte order: upstream put the first byte v1 in the HIGH 16 bits, which
    //     is mirrored relative to the working 4bpp path (first value -> LOW
    //     significance). On this S3 panel that flips the row left-to-right, so
    //     put v1 in the low half to match 4bpp.
    uint32_t shift = (mode == WHITE_ON_BLACK) ? 1u : 0u;
    ...
    for (uint32_t j = 0; j < EPD_WIDTH / 16; j++)
    {
        ...
        wide_epd_input[j] = (lut_1bpp[v1] << shift) | ((lut_1bpp[v2] << shift) << 16);
    }
}
...
{
    epd_start_frame();
    // LOCAL PATCH (foolscap): `skipping` is a global not reset by
    // epd_start_frame(). Calling this repeatedly for partial-height areas left
    // it nonzero, so the next frame's leading skipped rows went straight to
    // epd_skip() (no buffer zeroing) and painted the previous band's last
    // latched row down the whole column. Reset it so leading skips latch a
    // zeroed no-op row.
    skipping = 0;
    ...
    for (int32_t i = 0; i < EPD_HEIGHT; i++)
    {
        ...
        // LOCAL PATCH (foolscap): use write_row (not epd_output_row) so the
        // global `skipping` is reset to 0 while in the band, exactly like the
        // grayscale feed_display path. Otherwise the first below-band skipped
        // row goes straight to epd_skip() without zeroing the latch and paints
        // the last band row down the column (black bands below the cursor).
        write_row(time);
        if (shifted)
        {
            memset(line, 0, sizeof(line));
